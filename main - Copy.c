...
uint8_t private_key[ECC_PRIVATE_KEY_SIZE];         // Buffer for the private key
uint8_t public_key[ECC_PUBLIC_KEY_SIZE];           // Buffer for the public key (X || Y)
size_t private_key_len, public_key_len;            // To store key lengths
...
void generate_ecdsa_key_pair(void) {
	 // Initialize the ECC context using low-memory mode
    ...
    //cmox_ecdh(P_pEccCtx, P_CurveParams, P_pPrivKey, P_PrivKeyLen, P_pPubKey, P_PubKeyLen, P_pSharedSecret, P_pSharedSecretLen)
    ...
    if (retval == CMOX_ECC_SUCCESS) {
        HAL_UART_Transmit(&huart2, (uint8_t *)"ECDSA key pair generated successfully!\r\n", 40, HAL_MAX_DELAY);
        print_public_key_hex();
    } else {
        // Print detailed error message
        switch (retval) {
            case CMOX_ECC_ERR_MATHCURVE_MISMATCH:
                HAL_UART_Transmit(&huart2, (uint8_t *)"Error: Math curve mismatch\r\n", 28, HAL_MAX_DELAY);
                break;
            case CMOX_ECC_ERR_ALGOCURVE_MISMATCH:
                HAL_UART_Transmit(&huart2, (uint8_t *)"Error: Algorithm curve mismatch\r\n", 34, HAL_MAX_DELAY);
                break;
            case CMOX_ECC_ERR_BAD_PARAMETERS:
                HAL_UART_Transmit(&huart2, (uint8_t *)"Error: Bad parameters passed\r\n", 31, HAL_MAX_DELAY);
                break;
            case CMOX_ECC_ERR_WRONG_RANDOM:
                HAL_UART_Transmit(&huart2, (uint8_t *)"Error: Invalid or insufficient random input\r\n", 44, HAL_MAX_DELAY);
                break;
            case CMOX_ECC_ERR_MEMORY_FAIL:
                HAL_UART_Transmit(&huart2, (uint8_t *)"Error: Memory allocation failure\r\n", 34, HAL_MAX_DELAY);
                break;
            default:
                HAL_UART_Transmit(&huart2, (uint8_t *)"Unknown error occurred\r\n", 25, HAL_MAX_DELAY);
                break;
        }
    }
}

void print_public_key_hex(void) {
    char buffer[100];

    HAL_UART_Transmit(&huart2, (uint8_t *)"\r\nPublic Key X:\r\n", 17, HAL_MAX_DELAY);
    for (size_t i = 0; i < 32; i++) {
        snprintf(buffer, sizeof(buffer), "%02X", public_key[i]);
...
void print_key_pair(void) {
    char buffer[100];
    char buffer2[100];
    HAL_UART_Transmit(&huart2, (uint8_t *)"Private Key:\r\n", 14, HAL_MAX_DELAY);
    for (size_t i = 0; i < 5; i++) {
    	//HAL_UART_Transmit(&huart2, (uint8_t *)private_key_len, strlen(private_key_len), HAL_MAX_DELAY);
        snprintf(buffer, sizeof(buffer), "%02X ", private_key[i]);
        HAL_UART_Transmit(&huart2, (uint8_t *)buffer, strlen(buffer), HAL_MAX_DELAY);
    }
    //HAL_UART_Transmit(&huart2, (uint8_t *)"\r\n", 2, HAL_MAX_DELAY);
    ...
    HAL_Delay(100);
    for (size_t i = 0; i < 5; i++) {
        snprintf(buffer2, sizeof(buffer2), "%02X ", public_key[i]);
...
void send_public_key_to_pc(void) {
    ...
    // **Send Public Key as Raw Binary (64 bytes)**
    HAL_UART_Transmit(&huart2, public_key, ECC_PUBLIC_KEY_SIZE, HAL_MAX_DELAY);
...
void sign_message() {
    ...
    retval = cmox_ecdsa_sign(
        ...
        random_buffer, sizeof(random_buffer),         /* Secure random data buffer */
        private_key, sizeof(private_key),         /* Private key for signing */
        computed_hash, CMOX_SHA256_SIZE,          /* SHA-256 digest to sign (32 bytes) */
        computed_signature, &computed_size        /* Output signature buffer */
    );
...
void process_command(void) {
    ...
    if (strcmp(command_buffer, "GENKEY") == 0) {
        HAL_UART_Transmit(&huart2, (uint8_t *)"Generating new key pair...\r\n", 29, HAL_MAX_DELAY);
